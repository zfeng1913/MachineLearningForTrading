    ...
    def query(self, s_prime, r):
        ...
        if self.dyna != 0:
            ...
            for i in range(self.dyna):
                ...
                s_d, a_d, s_d_1, r_1 = self.experience[rand.randint(0, len(self.experience) - 1)]
                #s = random
                # s_d = rand.randint(0, self.num_states - 1)
                # #a = random
                # a_d = rand.randint(0, self.num_actions - 1)
                # #s' = infer from T[]
                # s_d_1 = np.argmax(self.T[s_d, a_d, :])
                # #r = R[s,a]
                # r_1 = self.R[s_d, a_d]

                #update_q_table
                self.q_table[s_d, a_d] = (1 - self.alpha) * self.q_table[s_d, a_d] + self.alpha * (
                        r_1 + (self.gamma * self.q_table[s_d_1, np.argmax(self.q_table[s_d_1])]))
